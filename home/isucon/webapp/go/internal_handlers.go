				...
				for _, r := range waitingRides {
					expectedScores = append(expectedScores, expectedScoreType{
						...
						expectedScore: calcExpectedScore(r, chairLocation, chairModelByChairName[chair.Model].Speed) +
							float64((time.Now().Sub(r.CreatedAt)).Nanoseconds())*0.00000001, // うまく待ってる時間を考慮したい
					})
				}
			}
...
func matchingComp(ctx context.Context, rides []Ride) error {
	...
	defer tx.Rollback()
	_, err = tx.ExecContext(ctx, fmt.Sprintf("UPDATE rides SET chair_id = ELT(FIELD(id%s)%s) WHERE id IN (?%s)", strings.Repeat(",?", len(rideIDs)), strings.Repeat(",?", len(chairIDs)), strings.Repeat(",?", len(rideIDs)-1)), b...)
	if err != nil {
